...
export default function Edit() {
  ...
  const { _id, name, type, color, price, wheel_size, description, status } = state;
  ...
  return (
    <div className="edit-wrapper">
      <div className="set-btn">
        <button type="button" onClick={() => navigate("/")} className="edit-back-btn">Go back</button>
      </div>
      <div className="edit-info">
        <div className="edit-top">
          <p className='top-name'>{baseValues.name.toUpperCase()}{" "} <span className='top-subtitle'>- {baseValues.type.toUpperCase()}{" "}({baseValues.color.toUpperCase()})</span></p>
        ...
        <div className="edit-bottom">
          ...
          {/* STATUS CHANGE */}
            <select className='element-status-select edit-status-select' name="select-status"
                  ...
                  >
                      {values.map((el, index) => {
                          ...
                          return <option key={index} value={el} className="element-option">{el.charAt(0).toUpperCase() + el.slice(1)}</option>})}
                  </select>
            <div className="set-btn">
              <button type="button" className="edit-back-btn edit-delete-btn" onClick={() => {
                deleteBicycleById(_id);
              navigate("/");
     Notify.success('Deleted!');}}>Delete</button>
            </div>
          </div>
      ...
      {/* FORM */}
      <form id="edit-form" className="edit-form" onSubmit={formik.handleSubmit} onReset={formik.handleReset}>
