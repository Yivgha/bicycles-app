...
// STATUS CHANGE
    ...
    return (
      <div className={[
    'element-wrapper',
          bicycle.status === "available" ? "available-border" : bicycle.status === "busy" ? "busy-border" : "unavailable-border",
        ].join(' ')}
        >
            <div className='close-icon' onClick={() => deleteBicycle(_id)}>
              <svg xmlns="http://www.w3.org/2000/svg" width="8" height="8" viewBox="0 0 10 10" fill="currentColor">
  <path d="M9 9L1 1M9 1L1 9" stroke="black" strokeWidth="2" strokeLinecap="round"/>
</svg>
          </div>
          <div className='element-top ' onClick={()=>navigate(`/edit/${_id}`, {state: bicycle})}>
              <div className='top-info'>
                  <p className='top-name'>{name.toUpperCase()}{" "} <span className='top-subtitle'>- {type.toUpperCase()}{" "}({color.toUpperCase()})</span></p>
              </div>
              <div><p className='top-id'>ID: {_id}</p></div>
          </div>
          <div className='element-bottom'>
              <div className='element-status-box'>
                  <p className='element-status'>STATUS:</p>
                  <select className='element-status-select' name="status-select"
